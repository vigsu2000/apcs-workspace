...
public class Elevator extends JPanel implements ActionListener,Serializable
{
	...
	public void setFloor(int f) {
		...
		// Problem 4:
		doorOpen = buttons[0] && !buttons[2] && (buttons[1] && !floorBits[0] || !buttons[1] && floorBits[0] && !floorBits[1]) || buttons[2] && floorBits[0] && floorBits[1] || !buttons[0] && buttons[1] && !buttons[2] && !floorBits[0]; 

		// Problem 5:
		direction[0] = direction[0] = !floorBits[0] && floorBits[1] && buttons[2] || floorBits[0] && !floorBits[1] && (buttons[1] || buttons[2]);



		direction[1] = (floorBits[1] && buttons[0]  || floorBits[0] && buttons[1] || !floorBits[1] && buttons[2] || buttons[2] && !buttons[0] && !floorBits[0]);

		// Problem 6:
		lightUp = !floorBits[1] && floorBits[0] || !floorBits [0] && floorBits[1] && buttons[2];


		lightDown =floorBits[1] && (floorBits[0] || !floorBits[0] && !buttons[2]);
	...
	{
		FileIO x= new FileIO();
		e=(Elevator)x.writeObject(filename,e);
	}
	...
	{
		FileIO x= new FileIO();
		Elevator e=(Elevator) x.readObject(filename);
		return e;
	
	}
	/** Initializes all transient fields using parameters **/
	public void initializeGUI(JButton sAct,JButton[] sButtons)
	{
		swingAct=sAct;
		swingButtons=sButtons;
		clock = new Timer(ACT_LAG_TIME,this);
	}
}
